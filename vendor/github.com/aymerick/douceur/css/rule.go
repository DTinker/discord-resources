...
var atRulesWithRulesBlock = []string{
	"@document", "@font-feature-values", "@keyframes", "@keyframes", "@media", "@supports",
}
...
func (rule *Rule) EmbedsRules() bool {
	if rule.Kind == AtRule {
		for _, atRuleName := range atRulesWithRulesBlock {
			// Had to patch around -webkit-keyframes ...
			if strings.HasSuffix(rule.Name, strings.TrimPrefix(atRuleName, "@")) {
				return true
			}
		}
	}
